...
class Transformer(nn.Module):
    ...
    def subsequent_masking(x):
        ...
        batch_size, seq_len = x.size()
        subsequent_mask = np.triu(np.ones(shape=(seq_len, seq_len)), k=1).astype(x.dtype)
        subsequent_mask = torch.tensor(subsequent_mask)
        subsequent_mask.unsqueeze(0).expand(batch_size, seq_len, seq_len)
...
if __name__ == '__main__':
    # embedding = nn.Embedding(num_embeddings=1017, embedding_dim=128)
    # encoder = TransformerEncoder(layers_count=2, d_model=128, heads_count=4, d_ff=256, dropout_prob=0.2)
    # decoder = TransformerDecoder(layers_count=2, d_model=128, heads_count=4, d_ff=256, dropout_prob=0.2)
    # transformer = Transformer(embedding, encoder, decoder)
    #
    # sources_example = torch.tensor([[1,2,3,4,0], [1,5,3,2,5], [5,4,0,0,0]])
    # inputs_example = torch.tensor([[1,2,3,4,0], [1,5,3,2,5], [5,4,3,4,5]])
    # outputs_example = transformer(sources_example, inputs_example)
#     # print(outputs_example)

    embedding = nn.Embedding(num_embeddings=1017, embedding_dim=128)
    encoder = TransformerEncoder(layers_count=2, d_model=128, heads_count=4, d_ff=256, dropout_prob=0.2, embedding=embedding)
    decoder = TransformerDecoder(layers_count=2, d_model=128, heads_count=4, d_ff=256, dropout_prob=0.2,
                                 embedding=embedding)
    sources_example = torch.tensor([[1, 2, 3, 4, 0], [1, 5, 3, 2, 5], [5, 4, 0, 0, 0]])

    inputs_example = torch.tensor([[1], [1], [5]])
    sources_mask = Transformer.pad_masking(sources_example, sources_example.size(1))
    memory_mask = Transformer.pad_masking(sources_example, 1)
    # inputs_mask = Transformer.subsequent_masking(inputs_example) | Transformer.pad_masking(inputs_example, inputs_len)
    memory = encoder(sources_example, sources_mask)
    state = decoder.init_decoder_state()

    for _ in range(8):
        outputs, state = decoder(inputs_example, memory, memory_mask, state=state)
        inputs_example = outputs.argmax(dim=2)

        print(inputs_example.shape)